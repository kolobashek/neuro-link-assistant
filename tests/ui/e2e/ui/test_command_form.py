...
class TestCommandForm:
    ...
    def test_command_autocomplete(self, driver):
        ...
        if suggestion_element:
            ...
            assert "открой" in user_input.get_attribute(
                "value"
    ...
    def test_command_filter_functionality(self, driver):
        ...
        visible_buttons = [
            btn
            for btn in commands_grid.find_elements(By.CLASS_NAME, "command-button")
            if btn.is_displayed()
        ]
        ...
        for button in visible_buttons:
            ...
            assert filter_text in button_text or any(
                alt in button_text for alt in ["открыть", "запусти"]
            )
