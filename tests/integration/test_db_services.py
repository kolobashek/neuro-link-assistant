...
from core.db.models import User  # AIModel и Task удалены
...
class TestUserService:
    def test_user_registration(self, db_session):
        ...
        user_data = {
            ...
            "password": "secure_password123",  # Будет хешироваться в сервисе
        }
        ...
        # Пароль должен быть хеширован, не должен совпадать с исходным
    ...
    def test_user_profile_update(self, db_session):
        ...
        assert updated_user.bio == "Test biography"
        ...
        assert user_from_db.display_name == "Test User"
        assert user_from_db.bio == "Test biography"
...
class TestTaskService:
    ...
    def test_task_lifecycle(self, db_session, test_user):
        ...
        updated_task = task_service.update_task(
            task.id, title="Updated Lifecycle Task", description="Updated description", priority=3
        )
...
class TestAIModelService:
    def test_model_registration_and_usage(self, db_session):
        ...
        retrieved_model = ai_service.get_model_by_id(model.id)
        ...
        assert any(m.id == model.id for m in active_models)
        ...
        updated_model = ai_service.update_model_configuration(model.id, updated_config)
        assert updated_model.configuration["temperature"] == 0.5
        assert updated_model.configuration["max_tokens"] == 4000
        assert updated_model.configuration["top_p"] == 0.9
