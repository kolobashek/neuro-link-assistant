...
def ensure_log_files_exist():
    ...
    log_files = [Config.SUMMARY_LOG_FILE, Config.DETAILED_LOG_FILE, Config.SYSTEM_LOG_FILE]
...
def clean_old_logs(max_age_days=30):
    ...
    for root, dirs, files in os.walk(Config.LOGS_DIR):
        for file in files:
            ...
            if file in [
                os.path.basename(f)
                for f in [Config.SUMMARY_LOG_FILE, Config.DETAILED_LOG_FILE, Config.SYSTEM_LOG_FILE]
            ]:
                continue
