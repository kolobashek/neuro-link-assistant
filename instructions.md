...
- tests/unit/core/common/filesystem/ (тесты файловой системы) ✅
...
- [x] **Определение новой архитектуры и принципов дизайна**
  - [x] Определить новую структуру каталогов
    ...
    ```
  - [x] Установить архитектурные принципы и паттерны
    - **Принципы SOLID**:
      ...
      - **L** (Liskov Substitution): Подклассы могут заменять родительские классы без изменения поведения
    ...
    - **Основные паттерны**:
      ...
      - **Синглтон** (Singleton): Для глобальных реестров и менеджеров
      - **Стратегия** (Strategy): Для взаимозаменяемых алгоритмов
      - **Наблюдатель** (Observer): Для системы оповещений
      - **Команда** (Command): Для инкапсуляции действий и операций
  - [x] Определить интерфейсы взаимодействия между компонентами
    - Использование абстрактных базовых классов для определения интерфейсов
    - Реестры компонентов для регистрации и поиска сервисов
    - Фабрики для создания экземпляров без привязки к конкретным реализациям
  - [x] Установить конвенции именования и кодирования
    ...
    - **Интерфейсы**: Префикс Abstract или суффикс Base/Interface (например, AbstractKeyboard, FileSystemBase)

- [x] **Настройка инфраструктуры тестирования**
  - [x] Настроить модульные тесты для каждого компонента
    - Реализована структура тестов по аналогии со структурой проекта
    - Созданы модульные тесты для подсистемы ввода
  - [x] Настроить интеграционные тесты для проверки взаимодействий
    - Определена структура для интеграционных тестов
    - Созданы интеграционные тесты для подсистемы ввода
    - Добавлена поддержка изоляции тестов от реальных устройств через моки
  - [x] Настроить автоматизированное тестирование (CI)
    - Обновлен файл GitHub Actions для запуска всех типов тестов
    - Добавлена настройка отчетов о покрытии кода
    - Обновлены параметры запуска для полного охвата тестами
  - [x] Стандартизировать подход к мокированию и заглушкам
    - Использование pytest и unittest.mock для создания моков
    - Стандартизированные фикстуры для общих зависимостей
    - Использование фабрик для создания тестовых данных

### 3.2. Уровень 2: Ядро системы

- [x] **Базовые компоненты**
  - [x] Реестр компонентов (component_registry.py)
  - [x] Обработчик ошибок (error_handler.py)
  - [x] Инициализатор системы (system_initializer.py)
  - [x] Менеджер плагинов (plugin_manager.py)

- [x] **Абстракции**
  - [x] Абстрактные классы для всех подсистем
  - [x] Интерфейсы для платформенно-зависимых компонентов
  - [x] Фабрики для создания конкретных реализаций
...
- [x] **Базовые классы и интерфейсы**
  - [x] Создание абстрактного класса `AbstractKeyboard`
  - [x] Создание абстрактного класса `AbstractMouse`
  - [x] Создание класса `InputController`
  - [x] Проверка через тесты интерфейсов

- [x] **Фабрика и реестр**
  - [x] Реализация `InputRegistry` для регистрации контроллеров
  - [x] Реализация фабричных методов в `factory.py`
  - [x] Создание тестов для фабрики и реестра

- [x] **Реализация для Windows**
  - [x] Реализация `WindowsKeyboard`
  - [x] Реализация `WindowsMouse`
  - [x] Проверка соответствия базовым интерфейсам

- [x] **Тесты**
  - [x] Тесты для базовых классов и интерфейсов
  - [x] Тесты для фабрики и реестра
  - [x] Тесты для конкретных реализаций
  - [x] Интеграционные тесты подсистемы

#### 3.3.2. Подсистема файловой системы

- [x] **Базовые классы и интерфейсы**
  - [x] Создание абстрактного класса `AbstractFileSystem`
  - [x] Создание интерфейса для операций с файлами
  - [x] Определение контрактов для файловых операций
  - [x] Определение абстрактных методов для работы с файлами и директориями ✅

- [x] **Фабрика и реестр**
  - [x] Реализация `FileSystemRegistry`
  - [x] Реализация фабричных методов для создания компонентов
  - [x] Тестирование регистрации и создания компонентов ✅

- [x] **Реализация для Windows**
  - [x] Реализация `Win32FileSystem`
  - [x] Реализация специфичных для Windows операций
  - [x] Интеграция с базовыми интерфейсами

- [x] **Тесты**
  - [x] Модульные тесты для базовых классов
  - [x] Модульные тесты для специфичных реализаций
  - [x] Интеграционные тесты для проверки операций ✅
...
1. **Файловая система**:
   ...
   - ✅ Добавлены методы для устранения дублирования в абстрактном классе
...
1. **Файловая система**:
   - [x] Создать абстрактный класс в core/common/filesystem/base.py
   - [x] Перенести реализации в core/platform/<os>/filesystem
   - [x] Обновить тесты для использования новой архитектуры ✅
   - [ ] Удалить устаревшие файлы после миграции

2. **Контроллеры ввода**:
   - [x] Создать полный набор абстракций в core/common/input
   - [x] Переместить реализации в core/platform/<os>/input
   - [x] Обновить тесты для использования новых компонентов
...
## 9. Дополнительные примечания по TDD (Test-Driven Development)

- **Рабочий процесс**:
  1. Написать тест, который не проходит (Red)
  2. Реализовать минимальный код для прохождения теста (Green)
  3. Улучшить код, сохраняя функциональность (Refactor)

- **Временные заглушки в тестах**:
  - Для модульных тестов необходимо создавать временные заглушки (мок-объекты) ✅
  - Заглушки должны реализовывать необходимый интерфейс для прохождения тестов ✅
  - После реализации реального кода заглушки можно заменить реальными объектами

- **Преимущества подхода**:
  - Тесты служат документацией ожидаемого поведения системы
  - Код разрабатывается с учетом тестируемости
  - Рефакторинг можно проводить с уверенностью благодаря тестам

Данный план будет обновляться по мере прогресса работы над проектом. Все члены команды должны следовать указанным принципам и методикам.

## 10. Уточнения по рефакторингу файловой системы

После выполненной работы по рефакторингу подсистемы файловой системы, стоит отметить следующие моменты:

### 10.1. Добавленные абстрактные методы

В базовый абстрактный класс `AbstractFileSystem` были добавлены следующие абстрактные методы:
- `file_exists` - проверка существования файла
- `get_file_modification_time` - получение времени последней модификации файла
- `list_directory_names` - получение списка имен поддиректорий
- `write_file` - запись содержимого в файл

### 10.2. Реализация в тестах и мок-объектах

В соответствии с принципами TDD:
- Сначала были обнаружены требуемые абстрактные методы
- Временно реализованы в мок-классе для прохождения тестов
- Тесты теперь проходят успешно, что позволяет перейти к следующему этапу

### 10.3. Последующие шаги для файловой системы

1. **Реализация абстрактных методов в основных классах**:
   - Добавить реализацию этих методов в класс `Win32FileSystem`
   - Убедиться, что методы работают корректно на платформе Windows

2. **Расширение тестового покрытия**:
   - Добавить более детальные тесты для новых методов
   - Проверить различные сценарии использования
   - Проверить обработку краевых случаев и ошибок

3. **Документирование**:
   - Добавить подробную документацию для новых методов
   - Обновить примеры использования

### 10.4. Интеграция с другими подсистемами

После завершения рефакторинга файловой системы необходимо:
- Обновить все зависимые компоненты для использования новых интерфейсов
- Проверить совместимость с существующими функциями
- Убедиться, что интеграционные тесты проходят успешно

## 11. Важные замечания по процессу разработки

### 11.1. Обязательное создание тестов до реализации

Все новые функции должны сначала описываться в тестах. Это помогает:
- Четко определить требуемое поведение
- Создать ясный контракт для компонента
- Обеспечить проверяемость кода

### 11.2. Обработка внешних зависимостей

При работе с внешними зависимостями (ОС, файловая система, сеть):
- Использовать абстракции и интерфейсы
- Создавать моки для тестирования
- Изолировать платформенно-зависимый код

### 11.3. Периодическая оценка покрытия кода

Регулярно проверять покрытие кода тестами:
- Использовать инструменты анализа покрытия
- Выявлять непокрытые участки кода
- Добавлять тесты для непокрытых участков

## 12. График реализации

### 12.1. Краткосрочные цели (1-2 недели)

- [x] Завершение рефакторинга подсистемы ввода
- [x] Начало рефакторинга файловой системы
- [x] Добавление абстрактных методов в базовый класс файловой системы ✅
- [ ] Реализация методов в конкретных классах файловой системы
- [ ] Устранение дублирования в файловой системе

### 12.2. Среднесрочные цели (3-4 недели)
...
### 12.3. Долгосрочные цели (2-3 месяца)
...
Данный документ является живым руководством по проекту и будет обновляться по мере прогресса. Все участники разработки должны следовать указанным принципам и методикам для обеспечения качества и поддерживаемости кода.
