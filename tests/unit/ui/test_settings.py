...
class TestSettings:
    ...
    def test_theme_settings(self, driver):
        ...
        theme_toggles = driver.find_elements(
            By.CSS_SELECTOR, ".theme-toggle, #theme-switch, .dark-mode-toggle"
        )
    ...
    def test_language_settings(self, driver):
        ...
        language_selectors = driver.find_elements(
            By.CSS_SELECTOR, ".language-selector, #language-switch, .lang-dropdown"
        )
        ...
        if len(language_selectors) > 0:
            ...
            if len(language_options) > 0:
                ...
                assert (
                    ...
                    or driver.find_element(By.TAG_NAME, "html").get_attribute("lang")
                    != initial_language
    ...
    def test_notification_settings(self, driver):
        ...
        notification_settings = driver.find_elements(
            By.CSS_SELECTOR, ".notification-settings, #notification-settings, .notifications-toggle"
        )
        ...
        if len(notification_settings) > 0:
            ...
            notification_toggles = driver.find_elements(
                By.CSS_SELECTOR,
                ".notification-toggle, .notification-checkbox, input[type='checkbox']",
            )
            ...
            if len(notification_toggles) > 0:
                ...
                ), "Состояние переключателя уведомлений не изменилось"
    ...
    def test_user_preferences(self, driver):
        ...
        settings_buttons = driver.find_elements(
            By.CSS_SELECTOR, ".settings-button, #user-settings, .preferences-button"
        )
        ...
        if len(settings_buttons) > 0:
            ...
            settings_container = WebDriverWait(driver, 5).until(
                EC.visibility_of_element_located(
                    (By.CSS_SELECTOR, ".settings-container, .preferences-modal, #settings-panel")
                )
            )

            # Проверяем наличие элементов управления настройками
            setting_controls = settings_container.find_elements(
                By.CSS_SELECTOR, "input, select, button, .setting-control"
            )
            assert len(setting_controls) > 0, "В настройках отсутствуют элементы управления"

            # Находим кнопку закрытия настроек
            close_buttons = settings_container.find_elements(
                By.CSS_SELECTOR, ".close-button, .close-modal, .back-button"
            )
            ...
            if len(close_buttons) > 0:
                ...
                WebDriverWait(driver, 5).until(
                    EC.invisibility_of_element_located(
                        (
                            By.CSS_SELECTOR,
                            ".settings-container, .preferences-modal, #settings-panel",
                        )
                    )
                )
    ...
    def test_api_key_settings(self, driver):
        ...
        api_key_fields = driver.find_elements(
            By.CSS_SELECTOR, "input[type='password'], .api-key-input, #api-key"
        )
        ...
        if len(api_key_fields) > 0:
            ...
            save_buttons = driver.find_elements(
                By.CSS_SELECTOR, ".save-button, #save-api-key, button[type='submit']"
            )
            ...
            if len(save_buttons) > 0:
                ...
                try:
                    ...
                    assert (
                        "сохранен" in success_message.text.lower()
                        or "успешно" in success_message.text.lower()
                        or "saved" in success_message.text.lower()
                    ), "Отсутствует сообщение об успешном сохранении API-ключа"
                except Exception:
    ...
    def test_history_settings(self, driver):
        ...
        clear_history_buttons = driver.find_elements(
            By.CSS_SELECTOR, ".clear-history, #clear-history-button, .history-clear-btn"
        )
        ...
        if len(clear_history_buttons) > 0:
            ...
            if initial_count > 0:
                ...
                history_rows_after = driver.find_elements(
                    By.CSS_SELECTOR, "#history-table tbody tr"
                )
                assert (
                    len(history_rows_after) < initial_count
                ), "Количество записей в истории не уменьшилось после очистки"
